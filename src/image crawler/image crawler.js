...
(async () => {
  ...
  await page.goto("https://bdocodex.com/tw/recipes/culinary/");

  try {
    // 點擊篩選器 更改顯示方式 從10改成200
    const select = await page.$('select[name="CulinaryRecipesTable_length"]');
    ...
    const rows = await page.$$("#CulinaryRecipesTable > tbody > tr");
    for (const row of rows) {
      ...
      // 這邊我們要下載素材圖片
      // 首先先取得名字
      const element = await row.$("td.dt-title > a > b");
      ...
      // 取得圖片連結
      const img = await row.$("td.dt-icon > div > a > div > img");
