...
(async () => {
  ...
  await page.goto("https://bdocodex.com/tw/items/materials/seed/");

  try {
    // 點擊篩選器 更改顯示方式 從10改成200
    const select = await page.$('select[name="MainItemTable_length"]');
    ...
    let flag = true;
    while (flag) {
      const nextButton = await page.$('#MainItemTable_next');
      const isDisabled = await page.$eval('#MainItemTable_next', button => button.classList.contains('disabled'));
